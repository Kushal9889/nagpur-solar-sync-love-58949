...
1. **Clone the repository**
   ```bash
   git clone https://github.com/Kushal9889/nagpur-solar-sync-love-58949.git
   cd nagpur-solar-sync-love-58949
   ```
...
3. **Configure environment variables**
   ...
   - `STRIPE_API_KEY`: Your Stripe **test mode** API key (starts with `sk_test_`)
   - `STRIPE_SIGNING_SECRET`: Webhook signing secret (get from Stripe CLI)
   - `MONGO_URL`: MongoDB connection string
   - `PORT`: Server port (default: 3000)
   - `NODE_ENV`: Environment mode (development/production)

   ⚠️ **IMPORTANT**: Always use Stripe **test mode** keys. Never use production keys in development.
